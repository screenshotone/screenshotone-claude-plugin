---
name: screenshot
description: Capture website screenshots, extract clean webpage Markdown, or check ScreenshotOne API usage with the hosted ScreenshotOne MCP server. Use for requested website captures, visual website analysis, content extraction, and capture quota checks.
---

# ScreenshotOne

Use the connected ScreenshotOne MCP tools for the user's requested website capture or content extraction. When invoked explicitly, use the user's request in $ARGUMENTS. If the request is missing a required URL, ask for it.

Discover the tools exposed by the ScreenshotOne server. Claude Code adds a plugin/server prefix to tool names; do not guess the prefix or substitute unrelated tools.
...
If the server needs authentication, guide the user to `/mcp`, select the ScreenshotOne plugin server, and complete browser sign-in and consent. Do not request API keys or tokens in chat or put them into configuration. The hosted endpoint is `https://mcp.screenshotone.com`, without a `/mcp` suffix.
...
- Images and screenshot-associated content are returned as temporary links. Open or download them with available tools when needed. Do not describe a screenshot's appearance without inspecting it. If no image-capable tool is available, return the links and explain the limitation.
